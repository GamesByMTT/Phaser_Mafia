...
export default class Disconnection extends Scene{
    ...
    create(){
        ...
        this.popupBackground = new Phaser.GameObjects.Sprite(this, gameConfig.scale.width/2, gameConfig.scale.height/2, "messagePopup").setScale(0.9);
        this.pageviewContainer.add(this.popupBackground)
         const disconnectionHeading = this.add.text(this.popupBackground.x, this.popupBackground.y - 250, "CONNECTION ERROR", {color:"#ffffff", fontFamily:"CarterOne", fontSize:"60px"}).setOrigin(0.5)
        const disconnectionText = this.add.text( this.popupBackground.x, this.popupBackground.y - 70, "Unable to connect to server", {color: "#ffffff", fontFamily:"CarterOne", fontSize: '50px', stroke: "#4f3130",
            strokeThickness: 1.5, align:"center", wordWrap: { width: 550, useAdvancedWrap: true }})
        disconnectionText.setOrigin(0.5);
        this.quit = new Phaser.GameObjects.Sprite(this, gameConfig.scale.width/2, gameConfig.scale.height/2 + 100, "disconnectClose").setInteractive().setScale(0.6)
