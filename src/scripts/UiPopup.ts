...
const TextStyle =  {color:"#ffffff", fontSize: "50px", fontFamily: 'CarterOne', align:"center"}
...
export class UiPopups extends Phaser.GameObjects.Container {
    ...
    menuBtnInit() {
        ...
        this.menuBtn.setPosition( gameConfig.scale.width / 1.10, gameConfig.scale.height * 0.12 );
    ...
    tweenToPosition(button: InteractiveBtn, index: number) {
       const targetY =  this.menuBtn.y + (index  * (this.menuBtn.height + 10))
    ...
    openSettingPopup() {
        ...
        const headingText = this.scene.add.sprite(0, -380, "settingText").setScale(0.9)
        const soundsImage = this.scene.add.sprite(-170, -120, "SOUNDS").setDepth(10).setScale(0.7);
        const musicImage = this.scene.add.sprite(-170, 80, "MUSIC").setDepth(10).setScale(0.7);

        const soundToggleButton = this.scene.add.image(150, -100, soundOn ? 'SoundtoggleOn' : 'SoundtoggleOff')
            .setInteractive()
            .setOrigin(0.5)
            .setScale(0.7);
        const musicToggleButton = this.scene.add.image(150, 100, musicOn ? 'MusictoggleOn' : 'MusictoggleOff')
            .setInteractive()
            .setOrigin(0.5)
            .setScale(0.7);
        ...
        soundToggleButton.on('pointerdown', () => {
            ...
            soundToggleButton.setTexture(soundOn ? 'SoundtoggleOn' : 'SoundtoggleOff');
        });
        ...
        this.settingClose.setPosition(500, -350);
    
        popupBg.setOrigin(0.5);
        popupBg.setScale(0.9);
        popupBg.setAlpha(1);
        
        infopopupContainer.add([popupBg, headingText, this.settingClose, soundsImage, musicImage, soundToggleButton, musicToggleButton,]);
    }
   ...
   // Function to adjust sound volume
    ...
    openLogoutPopup() {
        ...
        const popupText = this.scene.add.text(0, -80, "DO YOU REALLY WANT TO QUIT?", {color:"#ffffff", fontSize: "55px", fontFamily: 'CarterOne', align:"center" });
        ...
        this.yesBtn.setPosition(-130, 150).setScale(0.7);
        this.noBtn.setPosition(130, 150).setScale(0.7);
